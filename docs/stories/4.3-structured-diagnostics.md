...
- [ ] Every captured error includes: trigger source, tab URL (if available),
...
- [ ] Clearing errors via the popup or reset commands removes stored diagnostics
      and badge counts
- [ ] No PII or page content in diagnostic output (privacy-safe)
- [ ] New tests validate both the data structure and the UI flow
- [ ] Diagnostics storage capped to avoid bloating sync/state
