...
5. **E2E Tests** (`tests/e2e/popup-errors.spec.ts`):
   - `[ERROR_COPY_DETAILS]` - Copy details button copies markdown error report
   - `[ERROR_MARKDOWN_FORMAT]` - Validates report contains required sections
   - `[ERROR_DISMISS_CLEARS]` - Dismiss button clears errors and updates badge
   - `[ERROR_BADGE_SYNC]` - Badge count stays in sync with error count
