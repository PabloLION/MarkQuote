...
- Global setup runs `pnpm run build:e2e`, enabling the bridge handlers and diagnostics required by the suite, then patches the bundled manifest with `<all_urls>` host permission so selection injections succeed without a toolbar gesture.
